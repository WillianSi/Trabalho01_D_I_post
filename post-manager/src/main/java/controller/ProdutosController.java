...
public class ProdutosController extends HttpServlet {
		...
		try {
			if (dao.save(produto)) {
				...
				ControllerUtil.errorMessage(req, 
						"Produto '" + produto.getName() + "' não pode ser salvo.");
			}
	...
	try {
		if (dao.update(produto)) {
			ControllerUtil.sucessMessage(req, "Produto '" + produto.getName() + "' atualizado com sucesso.");
		}
		else {
			ControllerUtil.errorMessage(req, "Produto '" + produto.getName() + "' não pode ser atualizado.");
		}				
	} catch (ModelException e) {
		// log no servidor
		e.printStackTrace();
